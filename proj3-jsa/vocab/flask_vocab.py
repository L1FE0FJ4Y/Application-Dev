...
def check():
    ...
    if isMatch and isJumble and not (text in matchList):
        ...
        rslt = { "msg": "You already found {}".format(text), "flag": True, "wL": wdList }
    elif not isJumble:
        rslt = { "noJm": True, "msg": '"{}" can\'t be made from the letters {}'.format(text, jumble), "wL": wdList }
    elif not isMatch:
        rslt = { "msg": "{} isn't in the list of words".format(text), "wL": wdList }
    else:
